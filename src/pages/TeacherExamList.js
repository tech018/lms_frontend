...
const TeacherExamList = ({history, match}) => {
    ...
    const { loading:loadingDelete, error:errorDelete, success, examDelete } = deleteExam
    ...
    useEffect(()=> {
        ...
        if(userInfo && userInfo.role !== 1){
            ...
            dispatch(teacherexamlist(keyword, pageNumber, userInfo && userInfo.email))
        }
        if(error){
            toast.error(error)
        }
        ...
        userInfo, history, pageNumber, keyword, error,dispatch, errorDelete, success,examDelete
    ])
        ...
        <div style={{marginTop:'7rem'}}>
            <Container>
                <Row>
                    ...
                    <Col md={6}>
                        <Link to="/dashboard/teacher/examlist/create" style={{float:'right'}} className="btn btn-primary">Create Assessment Test</Link>
                    </Col>
                    <Col md={12}>
                    ...
                    {loading ? <Spinner animation="grow" /> :
                        ...
                        <Table className="mt-5" triped bordered hover>
                            ...
                            <tbody>
                                {exams.map(items=>(
                                    <tr key={items._id}>
                                    <td>{items.studentName}/{items.email}</td>
