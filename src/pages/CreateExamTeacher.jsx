...
import { CREATE_EXAM_PUBLIC_RESET } from '../constants/examConstants'
import { createteacherexam } from '../actions/examActions'
const CreateExamTeacher = ({history}) => {
    ...
    const createTeacherExam = useSelector(state => state.createTeacherExam)
    const { loading, error:errorCreate, exam, success } = createTeacherExam

    const total = Number(repetition) + Number(hesitation) + Number(selfCorrection) + Number(linking) + Number(express)
    const fluandcoh = Number(total/5).toFixed(1)
   

    const total2 = Number(wideRange) + Number(idiomatic) + Number(flexible) + Number(paraphrases)
    const lexires = Number(total2/4).toFixed(1)
    

    const total3 = Number(basicSentences) + Number(complex) + Number(error) + Number(comperehension)
    ...
    const grandtotal = Number(fluandcoh) + Number(lexires) + Number(gramandaccu) + Number(pronun)

    const finaltotal = Number(grandtotal/4).toFixed(1)

    const teacherEmail = userInfo && userInfo.email
    ...
    useEffect(()=> {
        ...
        if(success){
            ...
            history.push(`/dashboard/teacher/examlist`)
            dispatch({type:CREATE_EXAM_PUBLIC_RESET})
        }
        if(errorCreate){
            toast.error(errorCreate)
            dispatch({type:CREATE_EXAM_PUBLIC_RESET})
        }
    }, [userInfo,history, finaltotal, success, exam, errorCreate,dispatch])
    ...
    const handleSubmit = (e) => {
        ...
        if(!email){
            ...
            toast.error('Please enter TeacherEmail')
        }if(!studentName){
            toast.error('Please enter Studet Name')
        }else if(!repetition){
            ...
            toast.error('Please select Express and justify opinions, Topics developed logically')
        }else if(!wideRange){
                ...
                <Col md={12}>
                    ...
                    <Form style={{marginTop:'2rem'}} onSubmit={handleSubmit}>
                        <Row>
                            ...
                            <Col md={12}>
                                <h4 className="mt-3">Fluency and coherence (Score: {fluandcoh})</h4>
                                <hr />
                                <h5>Repetition</h5>
                                <Row>
                                    <Col md={4}>
                                ...
                                <h5 className="mt-2">Linking words and discourse Markers</h5>
                                <Row>
                                    <Col md={4}>
                                        <Form.Check
                                         value="1"
                                ...
                                <h5 className="mt-2">Express and justify opinions, Topics developed logically</h5>
                                <Row>
                                    <Col md={4}>
                                        <Form.Check
                                         value="2"
                            ...
                            <Col md={12}>
                                ...
                                <h5>if can use pronunciation features with precision and subtlety</h5>
                                <Row>
                                    <Col md={4}>
                                        <Form.Check
                                        type="radio"
